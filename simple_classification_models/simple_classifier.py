...
class depTSA(nn.Module):
    ...
    def compute_score(self, sent, lens):
        '''
...
#         sent = sent * weights_exp
        ...
        sents_vec = h.transpose(0, 1).contiguous().view(len(sent), -1)
    ...
    def forward(self, sent, label, lens):
        #Sent emb_dim + 50
        
        
        sent = F.dropout(sent, p=0.2, training=self.training)
        scores = self.compute_score(sent, lens)
        loss = nn.NLLLoss()
    ...
    def predict(self, sent, sent_len):
        #sent = self.cat_layer(sent, mask)
        scores = self.compute_score(sent, sent_len)
