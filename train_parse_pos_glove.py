...
cat_layer = WordPosMaskCat(config)
dp = dependency_path()
...
def get_context_weight(texts, targets, max_len):
    ...
    weights = np.ones([len(texts), max_len]) * 50#Fill the padding one as big
...
#     dg_test =data_generator(config, test_data, False)
    ...
    model_path = config.model_path+'parse_pos_model.pt'
    model = AspectSent(config)
#     if os.path.exists(model_path):
#         model = torch.load('data/models/parse_pos_model.pt')
    ...
    for e_ in range(config.epoch):
        ...
        for _ in np.arange(loops):
           ...
           #weights = get_dependency_weight(tokens, target_indice, max_len)#Get weights for each sentence
            ...
            sent_vecs = cat_layer(sent_vecs, mask_vecs, weights)#Batch_size*max_len*(2*emb_size)
            if config.if_gpu: 
...
def evaluate_test(dr_test, model):
    ...
    correct_count = 0
    while dr_test.index < dr_test.data_len:
        sent_vecs, mask_vecs, label_list, sent_lens, tokens = next(dr_test.get_ids_samples(True))
        target_indice = convert_mask_index(mask_vecs)#Get target indice
        max_len = max(sent_lens).item()
       #weights = get_dependency_weight(tokens, target_indice, max_len)#Get weights for each sentence
        weights = get_context_weight(tokens, target_indice, max_len)
        ...
        pred_label, _  = model.predict(sent_vecs, mask_vecs, sent_lens) 
