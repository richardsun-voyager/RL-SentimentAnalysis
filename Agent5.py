...
class Env(nn.Module):
    ...
    def get_current_state(self, current_input):
        '''
        ...
        '''
        intern_state = torch.cat([self.cell_state[0], self.hidden_state[0]], 1)
        #2*hidden_dim + 2 * word_dim
        
        self.observation = torch.cat([intern_state, current_input], 1)
...
class Policy_network(nn.Module):
    ...
    def forward(self, state):
        return F.softmax(self.state2action(state), 1)
...
class Agent(nn.Module):
    ...
    def forward(self, sents, masks, labels, texts):
        ...
        for i, word in enumerate(sent):
            ...
            #Get the state
            state = self.env.get_current_state(word_target)
            state = state.detach()

            p = self.pnet(state)#p is a tensor, 1*num
